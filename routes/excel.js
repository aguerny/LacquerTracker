...
app.post('/import', isLoggedIn, function(req, res) {
    if (mime.contentType(req.files.spreadsheet.path).startsWith("text/csv")) {
        ...
        fs.unlink(req.files.spreadsheet.path, function(err) {
            res.render('import.ejs', {title: 'Import Owned Polish - Lacquer Tracker', message:'Error: Filetype not .CSV'});
        })
    }
});
...
app.post('/admin/importnew', isLoggedIn, function(req, res) {
    if (mime.contentType(req.files.spreadsheet.path).startsWith("text/csv")) {
        ...
        reader.addListener('end', function(){
            fs.unlink(req.files.spreadsheet.path, function(err) {
                res.render('import.ejs', {title: 'Contact - Lacquer Tracker', message:'Could not send feedback. Please try again later.', inputname:req.body.name, inputemail:req.body.email, inputmessage:req.body.usermessage});
            })
        })
    } else {
        fs.unlink(req.files.spreadsheet.path, function(err) {
